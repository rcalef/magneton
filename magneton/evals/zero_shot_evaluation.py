...
def run_zero_shot_evaluation(
    ...
    # Set up paths
    proteingym_base = "/net/vast-storage/scratch/vast/kellislab/artliang/magneton/external/proteingym"
    ...
    for idx, row in tqdm(reference_df.iterrows(), total=len(reference_df), desc="Processing assays"):
        dms_id = row['DMS_id']
        ...
        try:
            ...
            dms_df[score_column] = dms_df['mutant'].map(lambda x: mutation_scores.get(x, np.nan))
            ...
            print(f"Processed {dms_id}: Spearman = {assay_metrics.get('spearman', np.nan):.4f}")
    ...
    performance_summary = calculate_performance_summary(all_results, reference_df, run_id)
    ...
    print(f"Mean Spearman correlation: {performance_summary.get('mean_spearman', np.nan):.4f}")
...
def score_mutations_lean(
    ...
    position_probs = score_positions(
        protein, protein_tensor, positions_to_score, model,
        needs_windowing, window_size, sequence
    )
...
def calculate_assay_metrics(df: pd.DataFrame, true_col: str, pred_col: str) -> Dict[str, float]:
...
def calc_ndcg_lean(y_true: np.ndarray, y_score: np.ndarray, top_percent: int = 10) -> float:
...
def calc_toprecall_lean(true_scores: np.ndarray, model_scores: np.ndarray, top_percent: int = 10) -> float:
...
def calculate_performance_summary(
    ...
    ndcg_values = [v['ndcg'] for v in valid_results.values() if 'ndcg' in v and not np.isnan(v['ndcg'])]
    auc_values = [v['auc'] for v in valid_results.values() if 'auc' in v and not np.isnan(v['auc'])]
    mcc_values = [v['mcc'] for v in valid_results.values() if 'mcc' in v and not np.isnan(v['mcc'])]
